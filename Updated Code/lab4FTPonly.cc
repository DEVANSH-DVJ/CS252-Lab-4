...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  int senderWindowSize = 37700;
  std::cout << "\nsenderWindowSize: " << senderWindowSize << std::endl;
  Config::SetDefault("ns3::TcpSocket::InitialCwnd",
                     UintegerValue(senderWindowSize)); // was 8
  Config::SetDefault("ns3::TcpSocket::RcvBufSize",
                     UintegerValue(senderWindowSize)); // was 65355
  ...
  // Create all types of point-to-point links that the topology needs (shown
  // above).
  ...
  BulkSendHelper source("ns3::TcpSocketFactory",
                        InetSocketAddress(i0i1.GetAddress(1), ftpSenderPort));
  ...
  PacketSinkHelper sinkFTP(
      "ns3::TcpSocketFactory",
      InetSocketAddress(Ipv4Address::GetAny(), ftpSenderPort));
  //       PacketSinkHelper sinkFTP ("ns3::TcpSocketFactory",InetSocketAddress
  //       (i0i1.GetAddress(1), ftpSenderPort));
  ...
  std::cout << std::endl
            ...
            << std::endl;
  std::cout << std::endl;
  // for loop with iterator is only required if there are multiple flows,
  // leaving it here just to show how it is used
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter =
    ...
    std::cout << "  Input Load\t\t"
              << iter->second.txBytes * 8.0 /
                     (iter->second.timeLastTxPacket.GetSeconds() -
                      iter->second.timeFirstTxPacket.GetSeconds()) /
                     1024
              << " Kbps" << std::endl;
    std::cout << "  Observed Throughput\t"
              ...
              << " Kbps" << std::endl;
    std::cout << "  Mean delay\t\t"
              << iter->second.delaySum.GetSeconds() / iter->second.rxPackets
              << std::endl;
